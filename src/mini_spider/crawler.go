...
type Crawler struct {
	urlTable		*UrlTable
	config 			*mini_spider_confif.BasicConfig
	queue			*Queue
...
func (c *crawler) Run() {
	for !c.stop {
		...
		if c.urlPattern.MatchString(task.url) {
			...
			if err != nil {
				log.Logger.Error("web_package.SavaWebPage(%s):%s", task.url, err.Error())
			} else {
...
func (c *Crawler) crawlChild(data []byte, task *CrawlTask ) error {
	...
	if err != nil {
		return fmt.Errorf("web_package.ParseWebPage():%s", err.Erro())
	}
